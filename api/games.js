  ...
  try {
    if (base() && process.env.SUPABASE_ANON_KEY) {
      const response = await fetch(
        `${base()}/rest/v1/tcg_games?active=eq.true&select=provider_game_id,name&order=name.asc`,
        { headers: headers() },
      );
      if (!response.ok) throw new Error("Indexed game catalog unavailable.");
      return res
        .status(200)
        .json({ source: "indexed-catalog", games: await response.json() });
    }
    if (!process.env.JUSTTCG_API_KEY)
      return res
        .status(503)
        .json({ error: "Catalog synchronization is not configured." });
    ...
    return res
      ...
      .json({ source: "provider", games: body.data ?? [] });
  } catch (error) {
