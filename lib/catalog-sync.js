import { parseProviderPrice } from "./supabase-catalog-repository.js";
...
export function normalizeCatalogCard(card, game, set) {
  ...
  const providerSetId = String(
    first(card.set, set?.provider_set_id, set?.id, set?.setId) ?? "",
  ).trim();
  const cardNumber = String(
    first(card.number, card.cardNumber, card.card_number, providerCardId) ?? "",
  ).trim();
  const variants = (Array.isArray(card.variants) ? card.variants : []).map(
    (variant) => {
      const condition = String(variant.condition ?? "").trim() || null;
      const printing = String(variant.printing ?? "").trim() || null;
      const language = String(variant.language ?? "").trim() || null;
      return {
        provider_variant_id:
          String(
            first(variant.uuid, variant.id, variant.tcgplayerSkuId) ?? "",
          ).trim() ||
          [printing, language, condition].filter(Boolean).join(":") ||
          null,
        condition,
        printing,
        language,
        ...
        price: parseProviderPrice(variant.price),
      };
    },
  );
  return {
...
export async function syncCatalog({
  ...
  const previous = repository.getLatestSync
    ? await repository.getLatestSync()
    : null;
  const resumeFromGame =
    previous?.status === "failed" ? Number(previous.games_processed) || 0 : 0;
  ...
  try {
    ...
    if (resumeFromGame)
      logger({
        resume_from_game: resumeFromGame,
        previous_sync: previous?.id,
      });
    for (const game of selectedGames) {
      if (gamesProcessed < resumeFromGame) {
        gamesProcessed += 1;
        continue;
      }
      const storedGame = await repository.upsertGame(game);
      const gameSets = await provider.getSets(game.provider_game_id ?? game.id);
      ...
      for (const set of gameSets) {
        ...
        storedSets.set(
          String(storedSet.provider_set_id ?? storedSet.id),
          storedSet,
        );
        sets += 1;
      }
      for await (const page of provider.iterCards({
        ...
        const normalizedCards = page.data.map((card) => {
          const set =
            storedSets.get(String(card.set ?? "")) ??
            storedSets.get(String(card.set_id ?? "")) ??
            null;
          const normalized = normalizeCatalogCard(card, game, set);
          normalized.game_id = storedGame.id;
          normalized.set_id = set?.id ?? null;
          return normalized;
        });
        if (repository.upsertCards)
          await repository.upsertCards(normalizedCards);
        else
          for (const card of normalizedCards) await repository.upsertCard(card);
