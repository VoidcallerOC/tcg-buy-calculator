...
export function createJustTcgProvider({
  ...
  function applyUsage(body) {
    ...
    if (!loggedUsage) {
      ...
      console.log(
        JSON.stringify({
          justtcg_plan: usage.apiPlan ?? null,
          justtcg_rate_limit: Number.isFinite(rateLimit) ? rateLimit : null,
          justtcg_daily_remaining: Number.isFinite(dailyRemaining)
            ? dailyRemaining
            : null,
          justtcg_page_size: currentPageSize,
          justtcg_delay_ms: currentDelayMs,
        }),
      );
    }
  }
